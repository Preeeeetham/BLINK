...
def run_benchmark(
    ...
    print(f"   • Evaluation Protocol   : Synthetic Continuous Simulation Ground Truth (N = {len(sub_timesteps)} frames)")
    print(f"   • Mean Synthesized PSNR : {mean_psnr:.2f} dB")
    print(f"   • Mean Linear Blend PSNR: {mean_lin_psnr:.2f} dB (Linear Interpolation Baseline)")
    print(f"   • PSNR Advantage Gain   : +{mean_psnr - mean_lin_psnr:.2f} dB over Baseline")
    print(f"   • Mean Synthesized SSIM : {mean_ssim:.4f}")
    print(f"   • Radiance Conservation : {mean_cons:.2f}% (Atmospheric Flux Regularization)")
